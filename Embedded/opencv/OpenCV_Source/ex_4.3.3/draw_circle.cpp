...
int main()
{
    ...
    circle(image, pt2, 60, magenta, FILLED, LINE_AA);
    circle(image, pt2, 60, magenta, FILLED, LINE_AA, 1);

    int font = FONT_HERSHEY_COMPLEX;
    putText(image, "center_blue", center, font, 1.2, blue);
    putText(image, "pt1_orange", pt1, font, 0.8, orange);
    putText(image, "pt2_magenta", pt2 + Point(2, 2), font, 0.5, Scalar(0, 0, 0), 2); // 글씨에 그림자 효과
    putText(image, "pt2_magenta", pt2, font, 0.5, magenta, 1);

    imshow("원그리기", image);
    setMouseCallback("원그리기", onMouse, 0);
    waitKey(0);
    return 0;
}