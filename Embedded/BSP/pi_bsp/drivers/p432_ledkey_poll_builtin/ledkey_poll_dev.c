...
static DECLARE_WAIT_QUEUE_HEAD(waitQueueRead); // 선언과 동시에 초기화
...
static ssize_t ledkey_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
    ...
    //	kbuf=gpioKeyGet();
    if (!(filp->f_flags & O_NONBLOCK)) // blocking mode open
    {
        wait_event_interruptible(waitQueueRead, keyNum); // keyNum이 참일때 깨어남
        //		wait_event_interruptible_timeout(waitQueueRead,keyNum,100);	//100*1Hz=1Sec
    }
    if (mutex_trylock(&keyMutex) != 0) {
        if (keyNum != 0) {
            kbuf = (char)keyNum;
...
static __poll_t ledkey_pool(struct file *filp, struct poll_table_struct *wait)
{
    unsigned int mask = 0;

    if (wait->_key & POLLIN)
        poll_wait(filp, &waitQueueRead, wait);
    if (keyNum > 0) // key pressed
        mask = POLLIN;
...
struct file_operations ledkey_fops = {
    ...
    .poll = ledkey_pool,   .release = ledkey_release,
};
