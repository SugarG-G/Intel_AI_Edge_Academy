...
static ssize_t ledkeydev_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
    ...
    if (!(filp->f_flags & O_NONBLOCK)) // BLOCK Mode
    {
        ...
        //  		wait_event_interruptible_timeout(WaitQueue_Read,sw_no,100); //100: 1/100 *100 =
        //  1Sec
    }
...
static void mkproc(void)
{
    ...
    keyledproc_led_fp = proc_create_data("led", S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO,
                                         keyledproc_root_fp, &proc_led_fops, NULL);
    keyledproc_key_fp = proc_create_data("key", S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO,
